...
var makefile = parse("Makefile", process.env);
console.log("Makefile: " + makefile.toString());
console.log(toTree(makefile, "tgz"));
var _plan = plan(makefile, ["tgz"]);
_plan.run();
...
//    find(targets: any[], target: string)
//    {
//        //console.log("FIND " + target);
//    return targets.find(function (candidate) { return candidate.target === target; });
//  }

//  init () {

//    var makefile = parse(this.makefile, process.env);

//    // Run!
//      return this.run(makefile, this.argv._).catch((err) => { Make.fail(argv.debug ? err.message : err.message);});
//  }

//  run (makefile: IMakefile, targets: string[]) {
//    this.debug('Run %s targets', targets.join(' '));
//    var argv: any = targets.concat();
...
//  alreadyExistsTarget(target: string)
//  {
//      return Promise.resolve();
//  }
//  noTarget (target:string) {
...
//  executeTarget(target: string)
//  {
//    target = path.resolve(this.dirname, target);
//    return new Promise((r, errback) => {
//      if (!this.find(this.targets, target)) {
//        if (fs.existsSync(target))
//          return this.alreadyExistsTarget(target);
//        return this.noTarget(target);
//      }

//      this.info('Invoking %s target', target);
//      var name = this.find(this.targets, target);
...
//  executeRecipe (target: string, name: string) {
//    return new Promise((r, errback) => {
//      var prerequities = target;
