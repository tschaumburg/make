...
// import { IAction } from "../planner";
// import { FileRef } from "../planner/plan/plan-impl";
// const os = require('os')

// export function runPlan(target: FileRef): void
// {
...
// function expandSymbols(cmd: string, target: FileRef, prerequisites: FileRef[], orderonlies: FileRef[]): string
// {
//     // $@:
//     // ===
...
//     cmd = cmd.replace(/\$@/g, target.orgname);
//     cmd = cmd.replace(/\$\(@D\)/g, path.dirname(target.fullname));
//     cmd = cmd.replace(/\$\(@F\)/g, path.basename(target.fullname));
...
//         cmd = cmd.replace(/\$</g, prerequisites[0].orgname);
...
//         cmd.replace(
//             /\$\?/g,
//             newerThan(target, prerequisites).map(p => p.orgname).join(' ')
//         );
